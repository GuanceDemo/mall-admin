    ...
    <template v-for="item in routes" v-if="!item.hidden&&item.children">
      ...
      <router-link v-if="hasOneShowingChildren(item.children) && !item.children[0].children&&!item.alwaysShow"
        ...
        <el-menu-item :index="item.path+'/'+item.children[0].path" :class="{'submenu-title-noDropdown':!isNest}">
          ...
          <span v-if="item.children[0].meta&&item.children[0].meta.title"
                slot="title">{{ menusTitle(item.children[0].meta.title) }}</span>
        </el-menu-item>
      </router-link>
      ...
      <el-submenu v-else :index="item.name||item.path" :key="item.name">
        <template slot="title">
          ...
          <span v-if="item.meta&&item.meta.title" slot="title">{{ menusTitle(item.meta.title) }}</span>
        </template>
        ...
        <template v-for="child in item.children" v-if="!child.hidden">
          ...
          <a v-else-if="child.path.startsWith('http')" v-bind:href="child.path" target="_blank" :key="child.name">
            <el-menu-item :index="item.path+'/'+child.path">
              ...
              <span v-if="child.meta&&child.meta.title" slot="title">{{ menusTitle(child.meta.title) }}</span>
            </el-menu-item>
          </a>
