<template>
  <div class="detail-container">
    <el-card shadow="never">
      <span class="font-title-medium">{{$t('language.returnedGoods')}}</span>
      <el-table
        border
        class="standard-margin"
        ref="productTable"
        :data="productList">
        <el-table-column :label="$t('language.productImage')" width="160" align="center">
        ...
        <el-table-column :label="$t('language.productName')" align="center">
          <template slot-scope="scope">
            ...
            <span class="font-small">{{$t('language.brand')}}：{{scope.row.productBrand}}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('language.price')+'/'+$t('language.goodsNo')" width="180" align="center">
          <template slot-scope="scope">
            <span class="font-small">{{$t('language.price')}}：￥{{scope.row.productRealPrice}}</span><br>
            <span class="font-small">{{$t('language.goodsNo')}}：NO.{{scope.row.productId}}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('language.attributes')" width="180" align="center">
          <template slot-scope="scope">{{scope.row.productAttr}}</template>
        </el-table-column>
        <el-table-column :label="$t('language.quantity')" width="100" align="center">
          <template slot-scope="scope">{{scope.row.productCount}}</template>
        </el-table-column>
        <el-table-column :label="$t('language.subtotal')" width="100" align="center">
          <template slot-scope="scope">￥{{totalAmount}}</template>
        </el-table-column>
      </el-table>
      <div style="float:right;margin-top:15px;margin-bottom:15px">
        <span class="font-title-medium">{{$t('language.totals')}}：</span>
        <span class="font-title-medium color-danger">￥{{totalAmount}}</span>
      </div>
    </el-card>
    <el-card shadow="never" class="standard-margin">
      <span class="font-title-medium">{{$t('language.serviceOrderInformation')}}</span>
      <div class="form-container-border">
        <el-row>
          <el-col :span="6" class="form-border form-left-bg font-small">{{$t('language.serviceOrderNo')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.id}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.applicationStatus')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.status | formatStatus}}</el-col>
        </el-row>
        <el-row>
          <el-col :span="6" class="form-border form-left-bg font-small" style="height:50px;line-height:30px">{{$t('language.orderNumber')}}
          </el-col>
          <el-col class="form-border font-small" :span="18" style="height:50px">
            {{orderReturnApply.orderSn}}
            <el-button type="text" size="small" @click="handleViewOrder">{{$t('language.view')}}</el-button>
          </el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.timeApplication')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.createTime | formatTime}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.userAccount')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.memberUsername}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.contactPerson')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.returnName}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.contactPhone')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.returnPhone}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.reasonReturn')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.reason}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.problemDescription')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.description}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6" style="height:100px;line-height:80px">{{$t('language.voucherPicture')}}
          </el-col>
      ...
      <div class="form-container-border">
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.orderAmount')}}</el-col>
          <el-col class="form-border font-small" :span="18">￥{{totalAmount}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6" style="height:52px;line-height:32px">{{$t('language.confirmRefundAmount')}}
          </el-col>
        ...
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6" style="height:52px;line-height:32px">{{$t('language.selectReceivingPoint')}}
          </el-col>
          <el-col class="form-border font-small" style="height:52px" :span="18">
        ...
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.nameConsignee')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{currentAddress.name}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.regions')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{currentAddress | formatRegion}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.detailedAddress')}}</el-col>
      ...
      <div class="form-container-border" v-show="orderReturnApply.status!==0">
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.processingPersonnel')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.handleMan}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.processingTime')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.handleTime | formatTime}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.processingRemarks')}}</el-col>
      ...
      <div class="form-container-border" v-show="orderReturnApply.status===2">
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.receivingPersonnel')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.receiveMan}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6" >{{$t('language.receivingTime')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.receiveTime | formatTime}}</el-col>
        </el-row>
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6">{{$t('language.receiptRemarks')}}</el-col>
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.receiveNote}}</el-col>
        </el-row>
      </div>
      <div class="form-container-border" v-show="orderReturnApply.status===0">
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6" style="height:52px;line-height:32px">{{$t('language.processingRemarks')}}</el-col>
      ...
      <div class="form-container-border" v-show="orderReturnApply.status===1">
        <el-row>
          <el-col class="form-border form-left-bg font-small" :span="6" style="height:52px;line-height:32px">{{$t('language.receiptRemarks')}}</el-col>
          <el-col class="form-border font-small" :span="18">
      ...
      <div style="margin-top:15px;text-align: center" v-show="orderReturnApply.status===0">
        <el-button type="primary" size="small" @click="handleUpdateStatus(1)">{{$t('language.confirmReturn')}}</el-button>
        <el-button type="danger" size="small" @click="handleUpdateStatus(3)">{{$t('language.rejectReturn')}}</el-button>
      </div>
      <div style="margin-top:15px;text-align: center" v-show="orderReturnApply.status===1">
        <el-button type="primary" size="small" @click="handleUpdateStatus(2)">{{$t('language.confirmReceipt')}}</el-button>
      </div>
    </el-card>
  </div>
</template>
<script>
  ...
  export default {
    ...
    filters: {
      formatStatus(status) {
        if (status === 0) {
          return $t('language.unprocessed');
        } else if (status === 1) {
          return $t('language.returning');
        } else if (status === 2) {
          return $t('language.completed');
        } else {
          return $t('language.rejected');
        }
      },
      formatTime(time) {
        if (time == null || time === '') {
    ...
    methods: {
      ...
      handleUpdateStatus(status){
        ...
        this.$confirm($t('language.sureOperation'), $t('language.tips'), {
          ...
          updateApplyStatus(this.id,this.updateStatusParam).then(response=>{
            this.$message({
              type: 'success',
              message: $t('language.operationSucceeded'),
              duration:1000
            });
            this.$router.back();
          });
