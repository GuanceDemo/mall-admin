<template>
  <div style="margin-top: 50px">
    <el-form :model="value"
             ...
             size="small">
      <el-form-item :label="$t('language.associatedTopics')+'：'">
        <el-transfer
          style="display: inline-block"
          filterable
          :filter-method="filterMethod"
          :filter-placeholder="$t('language.enterTopicName')"
          v-model="selectSubject"
          :titles="subjectTitles"
          :data="subjectList">
        </el-transfer>
      </el-form-item>
      <el-form-item :label="$t('language.associationSelection')+'：'">
        <el-transfer
          style="display: inline-block"
          filterable
          :filter-method="filterMethod"
          :filter-placeholder="$t('language.enterPreferredName')"
      ...
      <el-form-item style="text-align: center">
        <el-button size="medium" @click="handlePrev">{{$t('language.backFillAttributes')}}</el-button>
        <el-button type="primary" size="medium" @click="handleFinishCommit">{{$t('language.finishSubmitProduct')}}</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        subjectList: [],
        //专题左右标题
        subjectTitles: [$t('language.toBeSelected'), $t('language.isSelected')],
        //所有专题列表
        prefrenceAreaList: [],
        //专题左右标题
        prefrenceAreaTitles: [$t('language.toBeSelected'), $t('language.isSelected')]
      };
    },
    created() {
      this.getSubjectList();
